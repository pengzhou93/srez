...
class Model:
    ...
    def add_leaky_relu(self):
        with tf.variable_scope(self._get_layer_str()):
            out = tf.maximum(0.2*self.get_output(),self.get_output())
        self.outputs.append(out)
    def add_elu(self):
...
def _discriminator_model(sess,disc_input):
    ...
    model.add_conv2d(64, mapsize=mapsize, stride=1, stddev_factor=1.)
    model.add_leaky_relu()
